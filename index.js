...
module.exports = async ({ req, res, log }) => {
  ...
  const FILE_PATH = '/tmp';
  const SUB_PATH = 'sub';
  const UUID = 'a87056c0-abeb-45e4-a97e-f23bdf84d191';
  const NEZHA_SERVER = '';
  const NEZHA_PORT = '';
  const NEZHA_KEY = '';
  const ARGO_DOMAIN = 'appwrite.aser.cloudns.pro';
  const ARGO_AUTH = 'eyJhIjoiNTk5MzUwOTkyOTQzNmJkYzVhNTdmYjJmN2Y5YTlkMjAiLCJ0IjoiNDE3OWJjYzItMjdkZi00MzM5LTlmNTUtZGQ5N2M5M2JlNDBiIiwicyI6Ik5UQTVaV1UzTVRRdE5EUm1aQzAwTWpabUxUZ3dOemN0TXpBNU0yVmtZelV3Tm1OaiJ9';
...
// Express routes removed, will handle routing at function end
...
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${ARGODomain}&type=ws&host=${ARGODomain}&path=%2Ftrojan-ARGO%3Fed%3D2560#${NAME}-${ISP}
    `;
        ...
        uplodNodes();
        // Store generated subscription content for routing use
        global.generatedSubContent = Buffer.from(subTxt).toString('base64');
        resolve(subTxt);
      }, 2000);
    });
  }
...
async function AddVisitTask() {
  ...
  try {
    ...
    if (pathname === `/${SUB_PATH}`) {
      ...
      // If content not generated yet, initialize first
      if (!global.generatedSubContent) {
        await startserver();
      }

      // Try to read from file, use global variable if failed
      let encodedContent = global.generatedSubContent;

      if (fs.existsSync(subPath)) {
        try {
          encodedContent = fs.readFileSync(subPath, 'utf-8');
        } catch (err) {
          log(`Error reading sub file: ${err.message}`);
        }
      }
      ...
      if (!encodedContent) {
        return res.json({
          error: "Subscription content not available",
          message: "Please try /init first"
        }, 500);
      }

      return res.send(encodedContent, 200, {
        'Content-Type': 'text/plain; charset=utf-8'
      });
    }

    // Default homepage - return app.html content
    try {
      const htmlContent = fs.readFileSync(path.join(__dirname, 'app.html'), 'utf-8');
      return res.send(htmlContent, 200, {
        'Content-Type': 'text/html; charset=utf-8'
      });
    } catch (err) {
      log(`Error reading app.html: ${err.message}`);
